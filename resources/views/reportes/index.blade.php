...
<p>El puntaje obtenido por su empresa es:</p>
<div class="row">
    <div class="col-md-2 resultado-g" data-sema="{{$rCuestionario}}">
        {{$rCuestionario}}%
    </div>
    <div class="col-md-10" id="resultados" data-texto="{{$rCuestionario}}" style="font-size:1.0vw!important;">
    </div>
</div>
<div class="row">
...
<ul class="nav nav-pills nav-justified w-100" id="pills-tab" role="tablist">
  ...
  <li class="nav-item">
    <a class="dim_description nav-link {{$idx === 0 ? 'active' : ''}}" id="pills-tab-{{$dim->id}}" data-toggle="pill" href="#pills-{{$dim->id}}" role="tab" aria-controls="pills-{{$dim->id}}" aria-selected="{{$idx === 0 ? 'true' : ''}}">{{$dim->nombre}}</a>
  </li>
  @endforeach
</ul>
</div>
...
<div class="tab-content" id="pills-tabContent">
  @foreach($dimensiones as $idx => $dim)
  <div class="tab-pane fade {{$idx === 0 ? 'show active' : ''}}" id="pills-{{$dim->id}}" role="tabpanel" aria-labelledby="pills-tab-{{$dim->id}}" style="font-size:1.05vw!important;">{!!$dim->descripcion!!}</p></div>
  @endforeach
</div>
</div>
</div>
...
<div class="content-blanco">
<h5>Resultado por dimensiones</h5>
<p style="font-size: 1.05vw;">Haga click sobre alguna de las dimensiones para desplegar el resultado por indicadores.</p>
        ...
        <div class="card-header" role="tab" id="headingOne">
            <span data-toggle="collapse" href="#collapse{{$i}}" aria-expanded="true" aria-controls="collapse{{ $i }}">
                <div class="row">
                    ...
                    <div class="col-md-3" style="font-size: 1.2vw;  display: flex; justify-content: center; flex-direction: column;">
                        <strong> {{ $dimensiones[$i]->nombre }}: {{$niveles[$i]}} </strong>
                    </div>
                    <div class="col-md-7" style="font-size:1.05vw!important; display: flex; justify-content: center; flex-direction: column; overflow-y: auto">
        ...
        <div id="collapse{{$i}}" class="collapse indicadores-dimension" role="tabpanel" aria-labelledby="headingOne" data-parent="#accordion">
            <div class="card-body card-indicadores">
                <h5>Indicadores de la dimensión</h5>
                            <div class="row">
                            <div class="col-md-12">
                              <p style="font-size: 1.0vw;">Este gráfico de barras representa los puntajes obtenidos para cada uno de los indicadores dentro de la dimensión {{$dimensiones[$i]->nombre}}. Si desea conocer más información acerca de cada indicador, deslice el cursor sobre la barra y tendrá una breve descripción de cada uno de ellos.</p>
                                        <script type="text/javascript">
                                        FusionCharts.ready(function () {
...
<script type="text/javascript">
    ...
    var contentD = $('.resultado-d').attr("data-dime");
     ...
     else if (contentD >= 11 && contentD <= 40){
